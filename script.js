...
function addTask() {
  ...
  const taskDate = dateBox.value.trim();
  ...
  inputBox.value = "";
  // dateBox.value = ""; // Keep date so user can add more tasks to same date
    ...
    .forEach((date) => {
      ...
      grouped[date].forEach((task) => {
        ...
        requestAnimationFrame(() => {
          li.classList.add("adding-active");
...
listContainer.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("date-check")) {
    ...
    tasks = tasks.map((t) =>
      t.date === date ? { ...t, checked: !allChecked } : t
    );
  ...
  if (e.target.classList.contains("date-delete")) {
    ...
    const items = Array.from(listContainer.querySelectorAll("li")).filter(
      (l) =>
        l.querySelector("strong")?.textContent === date ||
        (l.dataset.index && tasks[l.dataset.index].date === date)
    );

    items.forEach((l) => l.classList.add("removing"));
  ...
  if (li && !e.target.closest(".task-actions")) {
